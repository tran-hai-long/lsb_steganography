...
def encode_page():
    ...
    if not verify_ascii(form.message.data):
        return render_template("lsb-encode.html", form=form, error="ASCII characters only.")
    ...
    if not channel:
        return render_template("lsb-encode.html", form=form, error="RGB or RGBA color channel only.")
...
def decode_page():
    ...
    channel: int = verify_channel(image)
    if not channel:
        return render_template("lsb-decode.html", form=form, error="RGB or RGBA color channel only.")
